...
function woo_bloyal_do_data_sync()
{
    ...
    foreach ($pending_to_push_inventory_changes as $key => $EntityChange) {
      if (isset($EntityChange->EntityUid) && isset($EntityChange->Entity->ProductUid) && isset($EntityChange->Entity->ProductCode)) {
        ...
        if ($sent_push_noticication_email) {
          ...
          wp_mail(array('adrian.morelos@akendos.com', 'info@ChefsEmporiumCT.com', 'melanie@thinkcreativegroup.com'), 'WooCommerce - Push inventory changes Debug', $msg, $headers);
        }
          ...
          if (($change != 0) && ($skip_changes == FALSE)) {
            ...
            if ($product != null) {
              ...
              if ($change > 0) {
                ...
                // update value
                $stock =  intval($woocommerce_inventory[$sku] + $change);
                ...
                update_post_meta( $product->id, '_stock_status', $status );
                ...
                $log .= print_r($product, true)."<br/><br/>";
              } else if (0 > $change) {
  ...
  */
  ...
  if ($response["status"] == "success") {
    ...
    $log .= "<b>EntitySyncEvents: </b> " . strval(json_encode($result->EntitySyncEvents)) . "<br>";
  } else {
